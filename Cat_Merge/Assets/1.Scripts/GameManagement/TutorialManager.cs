...
public class TutorialManager : MonoBehaviour, ISaveable
{
    ...
    private int lastMessageIndex = 0;                           // 마지막으로 본 메시지 인덱스
    ...
    public void StartTutorial()
    {
        ...
        // lastMessageIndex에 따라 시작 위치 결정
        currentMessageIndex = GetStartMessageIndex();
    ...
    {
        if (currentMessageIndex >= mainTutorialMessages.Length)
        {
            EndTutorial();
            return;
        }

        string currentMessage = mainTutorialMessages[currentMessageIndex];
    ...
    {
        if (isWaitingForUserAction) return;  // 유저 액션 대기 중이면 클릭 무시

        if (isTyping)
        {
            // 타이핑 중이라면 타이핑을 완료하고 전체 텍스트 표시
            if (typingCoroutine != null)
            {
                StopCoroutine(typingCoroutine);
            }
            tutorialText.text = mainTutorialMessages[currentMessageIndex];
            isTyping = false;
        }
        else
        {
    ...
    private int GetStartMessageIndex()
    {
        ...
        else if (lastMessageIndex <= 7)
        {
            return 4;
        }
        else if (lastMessageIndex <= 11)
        {
            return 8;
        }
        else if (lastMessageIndex <= 16)
        {
            return 13;
        }
        else if (lastMessageIndex <= 20)
        {
            return 17;
        }
        else if (lastMessageIndex <= 21)
        {
            return 21;
        }
    ...
    public void LoadFromData(string data)
    {
        ...
        this.lastMessageIndex = savedData.lastMessageIndex;
    }
